...
// Задает линию исп, для запроса прерываний
   ...
   int interruptLine = regData & 0xFF;
    ...
    //Если 255 - не используется, иначе - номер
    // номер вх контр прерываний для исп линии запроса
...
unsigned reg2Data=reg1Data >> 8;
   ...
   fprintf(out, "I/O Base: и I/O Limit: задает начальный и конечный адрес пространства в-в устройств расположенных за мостом(Только старшие 4 бита 16 битного адреса)\n");
    printf( "I/O Base: и I/O Limit: задает начальный и конечный адрес пространства в-в устройств расположенных за мостом(Только старшие 4 бита 16 битного адреса)\n");
if (IOBase==0){
...
unsigned long reg1Data=regData >> 4;
    ...
    printf("%#lx, ", reg1Data ); 
    fputs("Регистр базового адреса памяти", out);
    printf("Регистр базового адреса памяти");
 int typeBits = (regData >> 1) & 3;

    switch (typeBits) {
        case 0:
            fprintf(out," (Память может быть отражена в любое место 32-битного адрессного пространства)\n");
            printf(" (Память может быть отражена в любое место 32-битного адрессного пространства)\n");
            break;
        case 1:
            fprintf(out,"(Только ниже 1Мб)\n");
            printf("(Только ниже 1Мб)\n");
            break;
        case 2:
            fprintf(out,"(Память может быть отражена в любое место 64-битного адрессного пространства)\n");
            printf("(Память может быть отражена в любое место 64-битного адрессного пространства)\n");
            break;
        default:
...
//Расшивровка содержимого бащ адреса портов
void outputIOMemorySpaceBARData(long regData) {
  unsigned long reg1Data=regData-1;
    fprintf(out, "%#lx, ", reg1Data); 
    fputs("Регистр базового адреса портов \n", out);
    printf("%#lx, ", reg1Data);
    printf("Регистр базового адреса портов \n");
}
